...
const inputCls =
  ...
  'focus:outline-none focus:ring-2 focus:ring-red-400 focus:border-transparent transition-all duration-150';

const inputErrCls =
  'w-full px-3.5 py-2.5 rounded-xl border border-red-400 ' +
  'bg-red-50 dark:bg-red-900/10 text-gray-900 dark:text-white text-sm ' +
  'placeholder:text-gray-400 focus:outline-none focus:ring-2 focus:ring-red-400 transition-all duration-150';

// ─── Success screen ───────────────────────────────────────────────────────────

function SuccessScreen({ donor, onReset }: { donor: DonorRecord; onReset: () => void }) {
  ...
  return (
    <div className="max-w-md mx-auto px-4 py-16 text-center">
      ...
      <h2 className="text-2xl font-bold text-gray-900 dark:text-white mb-1">ডোনার নিবন্ধন সম্পন্ন!</h2>
      <p className="text-gray-500 dark:text-gray-400 text-sm mb-6">{donor.name}-এর তথ্য সফলভাবে সংরক্ষিত হয়েছে।</p>
      ...
      <div className="bg-white dark:bg-gray-900 rounded-2xl border border-gray-200 dark:border-gray-800 p-5 text-left space-y-3 mb-6 shadow-sm">
        {[
          ...
          { label: 'ইমেইল',        value: donor.email },
          {
            ...
            value: donor.lastDonatedDate ? format(parseISO(donor.lastDonatedDate), 'dd MMM yyyy') : 'প্রথমবার দেবেন',
          },
          ...(nextDate ? [{
          ...
          { label: 'অবস্থা', value: donor.isAvailableForDonation ? '✅ উপলব্ধ' : '⏸ অনুপলব্ধ' },
        ].map(({ label, value }) => (
...
export default function DonorRegistrationPage() {
  ...
  const {
    ...
    defaultValues: {
      ...
      bloodGroup: '', isAvailableForDonation: true, lastDonatedDate: '',
    },
  });
  ...
  const eligibility = useMemo(() => computeEligibility(watchedLastDonated), [watchedLastDonated]);

  // ── Mutation ───────────────────────────────────────────────────────────────
  
  const mutation = useMutation({
    mutationFn: (data: DonorFormData) =>
      api.post<{ success: boolean; donor: DonorRecord }>('/blood/donors', {
        ...data,
        age: Number(data.age),
    ...
    onSuccess: (donor) => setSavedDonor(donor),
    onError: (e: any) =>
  ...
  const handleReset = () => { reset(); setSavedDonor(null); };

  if (savedDonor) {
    return (
      <div className="min-h-screen bg-gray-50 dark:bg-gray-950">
        <MainNavbar /><SuccessScreen donor={savedDonor} onReset={handleReset} />
      </div>
    );
  }
        ...
        <div className="max-w-3xl mx-auto px-4 py-8 lg:mt-8">
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <p className="text-red-100 text-sm mt-0.5">Donor Registration — রক্ত দিন, জীবন বাঁচান</p>
            </div>
          </div>
        </div>
      </div>
            ...
            <div className="lg:col-span-2 space-y-5">
              ...
              <div className="bg-white dark:bg-gray-900 rounded-2xl border border-gray-200 dark:border-gray-800 p-5 shadow-sm space-y-4">
                <div className="flex items-center gap-2.5 pb-1">
                  ...
                  <h2 className="font-bold text-gray-900 dark:text-white text-sm">ব্যক্তিগত তথ্য</h2>
                </div>
                ...
                <Field label="বয়স" required error={errors.age?.message} hint="রক্তদাতার বয়স ১৮ থেকে ৬৫ বছরের মধ্যে হতে হবে">
                  <div className="relative">
                    <User className="absolute left-3 top-1/2 -translate-y-1/2 w-4 h-4 text-gray-400 pointer-events-none" />
                    <input
                      {...register('age', {
                        ...
                        validate: (v) =>
                          v === '' || (!isNaN(Number(v)) && Number(v) >= 18 && Number(v) <= 65)
                            ? true : 'সঠিক বয়স দিন (১৮–৬৫)',
                      })}
                      type="number" min={18} max={65} placeholder="যেমন: 25"
                    ...
                    <div className="relative">
                      ...
                      <input
                        {...register('phone', {
                          ...
                          pattern: { value: /^(?:\+?88)?01[3-9]\d{8}$/, message: 'সঠিক বাংলাদেশি নম্বর দিন' },
                        })}
              ...
              <div className="bg-white dark:bg-gray-900 rounded-2xl border border-gray-200 dark:border-gray-800 p-5 shadow-sm space-y-4">
                <div className="flex items-center gap-2.5 pb-1">
                  ...
                  <h2 className="font-bold text-gray-900 dark:text-white text-sm">রক্তদান তথ্য</h2>
                </div>
                  ...
                  <Controller
                    ...
                    render={({ field }) => (
                      <div className="grid grid-cols-4 sm:grid-cols-8 gap-2">
                        {BLOOD_GROUPS.map((bg) => (
                          <button type="button" key={bg} onClick={() => field.onChange(bg)}
                            className={`py-2.5 rounded-xl text-sm font-black transition-all duration-150 border-2 ${
                              field.value === bg
                                ? `${BG_STYLES[bg]} text-white border-transparent shadow-md scale-105`
                ...
                <Field label="সর্বশেষ রক্তদানের তারিখ" error={errors.lastDonatedDate?.message} hint="প্রথমবার রক্ত দিলে খালি রাখুন">
                  <div className="relative">
                    ...
                    <input
                      {...register('lastDonatedDate', {
                        validate: (val) => {
                          if (!val) return true;
                          if (isAfter(parseISO(val), new Date())) return 'ভবিষ্যৎ তারিখ দেওয়া যাবে না';
                          return true;
                        },
                      })}
                  ...
                  <div className={`flex items-start gap-3 p-3.5 rounded-xl border text-sm ${
                    ...
                    <div>
                      {eligibility.isEligible
                        ...
                        : <>
                            <p className="font-semibold">⏳ {eligibility.daysLeft} দিন পর যোগ্য হবেন</p>
                            <p className="text-xs mt-0.5 opacity-80">
                              পরবর্তী যোগ্য তারিখ: {eligibility.nextDate ? format(eligibility.nextDate, 'dd MMM yyyy') : '—'}
                            </p>
                          </>
                      }
                      <p className="text-xs mt-1 opacity-70">রক্তদানের মধ্যে কমপক্ষে {DONATION_GAP_MONTHS} মাসের বিরতি আবশ্যক।</p>
                    </div>
                  </div>
                )}

                <Field label="রক্তদানে উপলব্ধতা">
                  <Controller name="isAvailableForDonation" control={control}
                    render={({ field }) => (
                      <div className="grid grid-cols-2 gap-3">
                        {[
                          ...
                          <button type="button" key={String(value)} onClick={() => field.onChange(value)}
                            className={`p-3.5 rounded-xl border-2 text-left transition-all duration-150 ${
                              field.value === value
                                ? value ? 'border-green-500 bg-green-50 dark:bg-green-900/20' : 'border-gray-400 bg-gray-50 dark:bg-gray-800'
                                : 'border-gray-200 dark:border-gray-700 hover:border-gray-300 dark:hover:border-gray-600'
                            }`}
                          >
                            <p className={`text-sm font-semibold ${field.value === value ? (value ? 'text-green-700 dark:text-green-400' : 'text-gray-700 dark:text-gray-300') : 'text-gray-600 dark:text-gray-400'}`}>{label}</p>
              ...
              >
                {mutation.isPending
                  ? <><Loader2 className="w-5 h-5 animate-spin" /> সংরক্ষণ হচ্ছে...</>
                  : <><Heart className="w-5 h-5" /> ডোনার হিসেবে নিবন্ধন করুন <ArrowRight className="w-4 h-4 opacity-70" /></>
                }
              </button>
            </div>
            ...
            <div className="lg:col-span-1">
              <div className="sticky top-6 space-y-4">
                <div className="bg-white dark:bg-gray-900 rounded-2xl border border-gray-200 dark:border-gray-800 overflow-hidden shadow-sm">
                  <div className={`h-2 w-full transition-all duration-300 ${watchedBloodGroup ? BG_STYLES[watchedBloodGroup as BloodGroup]?.split(' ')[0] ?? 'bg-gray-200' : 'bg-gray-200 dark:bg-gray-700'}`} />
                  <div className="p-4">
                    <p className="text-xs font-semibold text-gray-400 uppercase tracking-wide mb-3">ডোনার প্রিভিউ</p>
                    <div className="flex items-center gap-3 mb-4">
                      <div className={`w-12 h-12 rounded-full flex items-center justify-center text-white font-black text-lg shrink-0 transition-all duration-300 ${watchedBloodGroup ? BG_STYLES[watchedBloodGroup as BloodGroup]?.split(' ')[0] : 'bg-gray-200 dark:bg-gray-700'}`}>
                    ...
                    <div className="space-y-2.5 text-xs">
                      {[
                        { icon: User,     label: 'বয়স',         value: watchedAge ? `${watchedAge} বছর` : '—',                                                color: 'text-gray-400' },
                        { icon: Phone,    label: 'ফোন',         value: watch('phone') || '—',                                                                  color: 'text-gray-400' },
                        { icon: Mail,     label: 'ইমেইল',       value: watch('email') || '—',                                                                  color: 'text-gray-400' },
                        { icon: Calendar, label: 'শেষ দান',     value: watchedLastDonated ? format(parseISO(watchedLastDonated), 'dd MMM yyyy') : 'প্রথমবার',  color: 'text-gray-400' },
                        ...(eligibility.nextDate ? [{ icon: Clock, label: 'পরবর্তী যোগ্য', value: format(eligibility.nextDate, 'dd MMM yyyy'), color: eligibility.isEligible ? 'text-green-500' : 'text-amber-500' }] : []),
                      ].map(({ icon: Icon, label, value, color }) => (
                        <div key={label} className="flex items-center gap-2">
                          <Icon className={`w-3.5 h-3.5 shrink-0 ${color}`} />
                          <span className="text-gray-400 w-20 shrink-0">{label}</span>
                          <span className="text-gray-700 dark:text-gray-300 truncate font-medium">{value}</span>
                        </div>
                      ))}
                    </div>
                    <div className="mt-3 pt-3 border-t border-gray-100 dark:border-gray-800">
                      <span className={`inline-flex items-center gap-1 px-2.5 py-1 rounded-full text-xs font-semibold ${watchedAvailable ? 'bg-green-100 text-green-700 dark:bg-green-900/30 dark:text-green-400' : 'bg-gray-100 text-gray-500 dark:bg-gray-800 dark:text-gray-400'}`}>
                ...
                <div className="bg-blue-50 dark:bg-blue-900/20 border border-blue-200 dark:border-blue-800 rounded-2xl p-4">
                  <p className="text-xs font-semibold text-blue-700 dark:text-blue-400 mb-2">📦 Database Fields</p>
                  <div className="space-y-1">
                    {['name','age','address','phone','email','bloodGroup','isAvailableForDonation','lastDonatedDate','nextEligibleDate','createdAt','updatedAt'].map((f) => (
                      <p key={f} className="text-xs font-mono text-blue-600 dark:text-blue-300">{f}</p>
                    ))}
                  </div>
                </div>
              </div>
