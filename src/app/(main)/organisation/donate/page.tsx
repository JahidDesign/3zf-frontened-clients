...
export default function DonateProgramPage() {
  // BUG FIX 1: Also pull `user` out of authStore so we can pass currentUserId to the modal.
  ...
  return (
    <div className="min-h-screen" style={{ background: 'var(--color-bg-secondary)' }}>
      ...
      {/* BUG FIX 2: DonateModal now uses `program` prop (renamed from `donation`),
          and we pass currentUserId so the "already voted" state shows correctly. */}
      <DonateModal
        ...
        onSuccess={() => {
          // Refresh the programs list after a successful donation
          api.get('/org/programs').then(({ data }) => setPrograms(data.data || [])).catch(() => {});
        }}
      />
