...
// ── Props: when used as modal (shop + onClose supplied)
//          when used as page  (neither supplied)
...
function CopyButton({ text }: { text: string }) {
  ...
  return (
    ...
    >
      {copied
        ? <><Check className="w-3.5 h-3.5 text-green-500" /> কপি হয়েছে</>
...
function KYCStatusView({ kyc, onReapply, isModal, onClose }: {
  kyc:       any;
  onReapply?: () => void;
  isModal:   boolean;
  onClose?:  () => void;
    ...
    <div className="space-y-5">
      ...
      <div className={`${s.bg} rounded-3xl p-6 ring-1 ${s.ring}`}>
        <div className="flex items-start gap-4">
          {kyc.photo?.url
            ? <img src={kyc.photo.url} alt="KYC Photo"
                className="w-20 h-20 rounded-2xl object-cover ring-2 ring-white dark:ring-gray-700 shrink-0" />
            : <div className="w-20 h-20 rounded-2xl bg-white dark:bg-gray-800 ring-2 ring-white dark:ring-gray-700
            ...
            {kyc.nidFront?.url && (
              <a href={kyc.nidFront.url} target="_blank" rel="noreferrer">
                <img src={kyc.nidFront.url} alt="NID Front"
                  className="rounded-xl w-full object-cover aspect-video hover:opacity-80 transition" />
      ...
      <div className="flex gap-3">
        {kyc.status === 'rejected' && onReapply && (
          <button onClick={onReapply} className="btn-primary flex-1 py-3.5 flex items-center justify-center gap-2">
        ...
        {isModal && onClose && (
          <button onClick={onClose}
            className="flex-1 py-3.5 rounded-2xl border border-gray-200 dark:border-gray-700
              ...
              hover:bg-gray-50 dark:hover:bg-gray-800 transition flex items-center justify-center gap-2">
...
// ─────────────────────────────────────────────────────────────
// Main Component — works as both page and modal
// ─────────────────────────────────────────────────────────────

export default function KYCPage({ shop, onClose }: KYCPageProps) {
  const isModal = !!onClose;           // true when used inside ShopList modal
  ...
  const {
    ...
    defaultValues: {
      ...
      region:        '',
      paymentMethod: 'bkash',
      paidAmount:    String(MEMBERSHIP_FEE),
    },
  });
  ...
  const resetAll = () => {
    reset(); setStep(1); setShowForm(false);
  ...
  const mutation = useMutation({
    mutationFn: async (data: KYCForm) => {
      ...
      // attach shopId if launched from a shop modal
    ...
    onSuccess: () => {
      ...
      if (isModal) {
        ...
        router.push('/supershop'); // navigate on standalone page
      }
    },
  ...
  // ── When used as modal, skip page chrome entirely ──────────
  // Wrap content differently depending on context
  ...
  // ── Page header (only for standalone page) ─────────────────
  const PageHeader = () => isModal ? null : (
    <div className="mb-6">
      <Link href="/supershop"
        className="inline-flex items-center gap-1.5 text-sm text-gray-500 dark:text-gray-400
          hover:text-blue-600 dark:hover:text-blue-400 transition mb-4">
  ...
  // ── Already has KYC — show status view ────────────────────
  ...
  return (
    <Content>
      ...
      {user && !isLoading && (!myKYC || showForm) && (
        <>
          {/* Shop context banner — only in modal mode */}
          {isModal && shop && (
            <div className="bg-teal-50 dark:bg-teal-900/20 border border-teal-200 dark:border-teal-800
              ...
              {shop.coverPhoto?.url
                ? <img src={shop.coverPhoto.url} alt={shop.name}
                    className="w-9 h-9 rounded-xl object-cover shrink-0" />
                : <div className="w-9 h-9 rounded-xl bg-teal-100 dark:bg-teal-900/40 shrink-0" />
              }
              <div className="min-w-0">
                <p className="text-xs text-teal-600 dark:text-teal-400 font-semibold">সদস্যপদের আবেদন</p>
                <p className="text-sm font-black text-gray-900 dark:text-white truncate">{shop.name}</p>
              </div>
            </div>
          )}

          {/* Promo banner — standalone page only */}
          {!isModal && step === 1 && !myKYC && (
            ...
            {STEPS.map((s, i) => {
              ...
              return (
                <div key={s.id} className="flex items-center gap-0.5 flex-1 last:flex-none">
                  ...
                  {i < STEPS.length - 1 && (
                    <div className={`flex-1 h-0.5 mb-4 mx-0.5 rounded transition-all ${
            ...
            <form onSubmit={handleSubmit(onSubmit)}>
              ...
              <div className="flex gap-3 mt-8">
                {step > 1 ? (
                  ...
                  // Step 1 back button in modal → close
